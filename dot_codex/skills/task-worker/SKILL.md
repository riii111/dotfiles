...
実装中は変更箇所に対応するテスト・検査を先に実行し、修正のたびに全検証を繰り返さない。
差分がまとまった時点で所定の検証を行う。失敗時やhead変更後は影響に応じて再実行し、最終headで必要な検証が済んでいることを確認する。
レビュー指摘は可能な範囲でまとめて修正・検証してからpushし、不要なCI再実行を避ける。
