...
The output directory contains `manifest.json`, `report.md`, `results.jsonl`, `events.jsonl`, and the saved answers. The manifest records task/skill hashes, target `HEAD`, Codex version, model settings, budgets, and the disabled Fast mode. A benchmark passes the remaining time and credit budget into each run and stops before another run when the observed limits are reached; billing limits are not guaranteed because usage and interruption events can arrive late. Report medians and reductions use only naturally completed runs; interrupted, failed, and timed-out runs remain visible but are excluded. `estimated_active_seconds` subtracts only observed parent-thread tool waits, so it is not pure model inference time. If credits or token usage are unavailable, the report says `unknown` or `partial` rather than treating the missing value as zero. Use `--allow-unmetered` only when continuing despite missing usage is intentional.
