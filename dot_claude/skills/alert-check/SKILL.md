...
`retryCheck` は caller 文脈が取れれば `same caller` / `same tenant` まで見るが、**失敗した個別リクエストの再試行結果そのものを保証するものではない**。

- `verdict: "recovered_same_caller"` → 同じ endpoint で tenant + caller が一致する後続成功がある。もっとも確度が高いが、それでも trace 再実行そのものの保証ではない
- `verdict: "recovered_same_tenant"` → 同じ endpoint で tenant 一致の後続成功がある。caller 固有の回復とは限らない
- `verdict: "recovered_endpoint_only"` → 後続成功はあるが、tenant/caller 一致は未確認
- access log 自体に caller 情報がなくても、同じ trace の Request/Response ログから補える場合がある
- endpoint-only は caller 情報が取れないケースだけでなく、後続成功の tenant/caller が source と一致しないケースも含む
