...
acquire_lock() {
	...
	mkdir -p "$MUX_LOCK_DIR"
	chmod 700 "$MUX_LOCK_DIR"
...
docker_exec_container() {
	...
	read -r -a exec_args <<<"$exec_cmd"
	(cd "$DOCKER_DIR" && docker compose -f "${DOCKER_COMPOSE_FILE}" exec "$container" "${exec_args[@]}")
}
...
stop_docker() {
	...
	session_exists && {
		tmux send-keys -t "${SESSION_NAME}:docker" C-c 2>/dev/null || true
		sleep 2
	}
