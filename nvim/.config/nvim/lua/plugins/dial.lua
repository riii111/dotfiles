return {
  ...
  keys = {
    ...
    { "g<C-a>", function() require("dial.map").manipulate("increment", "gnormal") end, mode = "n", desc = "Increment (sequence)" },
    { "g<C-x>", function() require("dial.map").manipulate("decrement", "gnormal") end, mode = "n", desc = "Decrement (sequence)" },
  ...
  config = function()
    ...
    -- Default augends: numbers, dates, booleans, case conversion, etc.
    local default = {
    ...
    config.augends:on_filetype({
      python = vim.iter({ default, { augend.constant.new({ elements = { "True", "False" }, cyclic = true }) } }):flatten():totable(),
    })

    -- Markdown: header levels
    config.augends:on_filetype({
      markdown = vim.iter({ default, { augend.misc.alias.markdown_header } }):flatten():totable(),
    })

    -- TypeScript/JavaScript: let/const
    local typescript_group = vim.iter({
      default,
      { augend.constant.new({ elements = { "let", "const" } }) },
    }):flatten():totable()

    vim.iter({ "typescript", "javascript", "typescriptreact", "javascriptreact", "tsx", "jsx" }):each(function(ft)
      config.augends:on_filetype({ [ft] = typescript_group })
    end)
  end,
}
