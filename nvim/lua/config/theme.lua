...
local function apply_flate_arc_italic()
	...
	local highlights = {
		...
		TabLine = { fg = colors.base.fg_alt, bg = "NONE" },
		TabLineFill = { bg = "NONE" },
...
-- BufferLine group labels and buffer highlights
-- These are auto-generated by bufferline, so we override after bufferline renders
local function apply_bufferline_overrides()
	local label_fg = colors.base.fg_dark
	local inactive_fg = colors.base.fg_alt

	-- Language group definitions: { prefix, color }
	local lang_groups = {
		{ "Go", colors.languages.go },
		{ "Rs", colors.languages.rust },
		{ "Lua", colors.languages.lua },
		{ "Ts", colors.languages.typescript },
		{ "Tsx", colors.languages.tsx },
		{ "Js", colors.languages.javascript },
		{ "Jsx", colors.languages.jsx },
		{ "C", colors.languages.c },
		{ "Cpp", colors.languages.cpp },
		{ "Kotlin", colors.languages.kotlin },
		{ "Docs", colors.languages.docs },
	}

	for _, group in ipairs(lang_groups) do
		local prefix, lang_color = group[1], group[2]
		-- Label: dark text on language-colored background
		vim.api.nvim_set_hl(0, "BufferLine" .. prefix .. "Label", { fg = label_fg, bg = lang_color })
		-- Selected: language color, bold
		vim.api.nvim_set_hl(0, "BufferLine" .. prefix .. "Selected", { fg = lang_color, bold = true, italic = true })
		-- Non-selected (inactive): gray color
...
vim.api.nvim_create_autocmd({ "BufEnter", "BufWinEnter" }, {
	callback = function()
		vim.defer_fn(apply_bufferline_overrides, 10)
	end,
	group = vim.api.nvim_create_augroup("BufferLineLabelFix", { clear = true }),
})
